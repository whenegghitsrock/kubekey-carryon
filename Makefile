...
REGISTRY ?= docker.io/whenegghitsrock
PROD_REGISTRY ?= docker.io/whenegghitsrock
...
.PHONY: kk
kk:
	CGO_ENABLED=0 go build -trimpath -tags "$(BUILDTAGS)" -ldflags "$(LDFLAGS)" -o $(BIN_DIR)/kk github.com/whenegghitsrock/kubekey-carryon/v3/cmd/kk;
...
manager-capkk: ## Build the capkk manager binary into the ./bin folder
	go build -trimpath -ldflags "$(LDFLAGS)" -o $(BIN_DIR)/manager github.com/whenegghitsrock/kubekey-carryon/v3

.PHONY: manager-k3s-bootstrap
manager-k3s-bootstrap: ## Build the k3s bootstrap manager binary into the ./bin folder
	go build -trimpath -ldflags "$(LDFLAGS)" -o $(BIN_DIR)/k3s-bootstrap-manager github.com/whenegghitsrock/kubekey-carryon/v3/bootstrap/k3s

.PHONY: manager-k3s-control-plane
manager-k3s-control-plane: ## Build the k3s control plane manager binary into the ./bin folder
	go build -trimpath -ldflags "$(LDFLAGS)" -o $(BIN_DIR)/k3s-control-plane-manager github.com/whenegghitsrock/kubekey-carryon/v3/controlplane/k3s
